...
class StochasticScalingPSO(Solver[List[float], float]):
    ...
    def step(self) -> None:
        ...
        # Create a quick lookup array so each dimension knows its group
        dim_to_group = [0] * self.problem.dimension
        for g_idx, group in enumerate(groups):
            for d in group:
                dim_to_group[d] = g_idx

        for i in range(self.swarm_size):
            # 1. Generate new r1 and r2 for each group, SPECIFIC TO THIS PARTICLE
            group_r1 = [random.random() for _ in range(k)]
            group_r2 = [random.random() for _ in range(k)]

            for d in range(self.problem.dimension):
                # 2. Look up the group index for this dimension
                g_idx = dim_to_group[d]
                
                # 3. Apply the particle's group-specific scalars
                r1 = group_r1[g_idx]
                r2 = group_r2[g_idx]
