...
class DE_AX(Solver[List[float], float]):
    """
    A canonical Differential Evolution (DE) solver for single-objective
    optimization.

    This is a `DE/rand/2/arith` implementation. It creates a trial vector for
    ...
    def step(self) -> None:
        ...
        for i in range(self.population_size):
            ...
            # 2. Recombination (Create Trial Vector) - Arithmetic crossover AX
            trial_vector = [0.0] * self.problem.dimension # create a trial vector for every dimension
            
            for j in range(self.problem.dimension):
                # sample a gamma  from a uniform distribution over [0,1] for every dimension
                gamma = random.random()
                # apply the arithmetic cross over formula
                trial_vector[j] = (1.0 - gamma) * target_vector[j] + gamma * donor_vector[j]
