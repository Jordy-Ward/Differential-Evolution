...
class DE_SPX(Solver[List[float], float]):
    """
    A canonical Differential Evolution (DE) solver for single-objective
    optimization.

    This is a `DE/rand/2/SPX` implementation. It creates a trial vector for
    ...
    def step(self) -> None:
        ...
        for i in range(self.population_size):
            ...
            # 2. Recombination (Create Trial Vector) - SPX
            # Parent, mutant, and a random selected vector x3 != to parent
            x1 = target_vector
            x2 = donor_vector
            ...
            # spx parameters
            m = 3
            epsilon = math.sqrt(m) # expansion rate for 3 parents
            
            # calculate the center of mass
            O = [(x1[j] + x2[j] + x3[j]) / 3.0 for j in range(dim)]
            
            # calculate the expanded vertices
            y1 = [O[j] + epsilon * (x1[j] - O[j]) for j in range(dim)]
            y2 = [O[j] + epsilon * (x2[j] - O[j]) for j in range(dim)]
            y3 = [O[j] + epsilon * (x3[j] - O[j]) for j in range(dim)]
            ...
            # assemble child vectors using calc weights
            trial_vector = [0.0] * dim
            for j in range(dim):
                trial_vector[j] = w1 * y1[j] + w2 * y2[j] + w3 * y3[j]
